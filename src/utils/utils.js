...
import { useDictionary, useUserStore } from '@/store'
...
// 字典映射
export async function setDictionary (list, code, key) { // 映射数组、字典code、映射数组key
  return new Promise((resolve, reject) => {
    const dictionary = useDictionary()
    let codeList = []
    const newList = []
    dictionary.getDictionary(code).then(res => {
      codeList = res
      for (const itemKey in list) {
        const item = list[itemKey]
        codeList.forEach(em => {
          if (eval(`item.${key}`) === em.value) {
            item.label = em.label
          }
        })
        newList.push({ ...item, keyName: itemKey })
      }
      resolve(newList)
    })
  })
...
export function downloadEvt (url) {
  const user = useUserStore()
  const { token } = user.$state
  const el = document.createElement('a')
  el.style.display = 'none'
  el.setAttribute('target', '_blank')
  el.href = `${url}?token=${token}`
...
/* 获取开始时间和结束时间 */
export function getStarEnd (time) {
  if (!time) {
    return false
  }
  const star = dayjs(time[0]).format('YYYY-MM-DD HH:mm:ss')
...
export function uploadFile (options = {}) {
  const { multiple, fileType, isServer, limit, isMessage = true } = options
  return new Promise((resolve, reject) => {
    try {
...
/* 新开页签跳转 */
export const openNewPage = ({ path, query = {} }) => {
